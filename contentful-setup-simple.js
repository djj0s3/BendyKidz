...
console.log('Starting Contentful setup with:');
...
async function setupContentful() {
  try {
    ...
    console.log('Successfully connected to Contentful space and environment');
    
    // Create content types
    await createContentTypes(environment);
    
    // Wait for content types to process
    console.log('Waiting for content types to process...');
    await new Promise(resolve => setTimeout(resolve, 5000));
    
    // Create assets and entries
...
async function createContentTypes(environment) {
  ...
  for (const contentType of contentTypes) {
    try {
      ...
      // Try to get the content type (check if it exists)
      try {
...
async function createContent(environment) {
  console.log('Creating assets and entries...');
  ...
  for (const [categoryId, categoryData] of Object.entries(categories)) {
    await createOrUpdateEntry(environment, 'category', `category-${categoryId}`, categoryData);
  }
  ...
  for (const article of articles) {
    ...
    await createOrUpdateEntry(environment, 'article', id, articleData);
  }
  ...
  for (const testimonial of testimonials) {
    ...
    await createOrUpdateEntry(environment, 'testimonial', id, testimonialData);
  }
